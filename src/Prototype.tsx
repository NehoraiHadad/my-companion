...
function loadState(): GameState {
  try {
    ...
    if (!fresh) {
      ...
      if (saved?.version === 5) {
        return applyElapsed({ ...defaultState, ...saved, memories: saved.memories ?? [], animationSlots: saved.animationSlots ?? {}, aiCharacter: saved.aiCharacter ?? false, characterVariants: saved.characterVariants ?? {}, decorations: saved.decorations ?? {}, claimedMilestones: saved.claimedMilestones ?? [], sourcePhoto: saved.sourcePhoto ?? saved.photo });
      }
...
function HomeScreen({ game, characterUrl, health, stage, evolution, day, needs, lowestNeed, callNeed, currentRoom, reaction, reactionId, wanderX, effect, isNight, aiReady, isSpeaking, activeMotion, clipUrl, onSettings, onSpeak, onAction, onUseMedicine }: any) {
  ...
  return <section className="home-screen">
    ...
    <div className="room-scene">
      ...
      {(Object.keys(decorMeta) as DecorKey[]).filter((key) => game.decorations[key]).map((key, index) => { const DecorIcon = decorIcons[key]; return <motion.div className={`room-decor decor-${key}`} key={key} aria-hidden initial={{ opacity: 0, scale: .5, y: -8 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ delay: index * .07, type: "spring", stiffness: 220, damping: 17 }}><DecorIcon /></motion.div>; })}
