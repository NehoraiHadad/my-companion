...
import {
  BackpackIcon, BellIcon, CameraIcon, CheckIcon, ChevronLeftIcon, ChevronRightIcon,
...
import { buildCharacterPrompt, buildOpenRouterCharacterRequest, characterStorageKey, characterVisuals, type CharacterVisual } from "./characterDirector";
import {
...
import {
  HOUR, absenceMessage, ageDay, applyElapsed, clamp, currentStage, defaultState, localDayKey,
  nextStage, performCareAction, stageMeta, useInventoryItem, whole,
  type ActionKey, type CharacterKind, type CompanionMotion, type GameState, type ItemKey, type NeedKey, type PersonalityId, type StageId, type ThemeId,
} from "./gameEngine";
...
function loadState(): GameState {
  try {
    ...
    if (!fresh) {
      ...
      if (saved?.version === 5) {
        return applyElapsed({ ...defaultState, ...saved, memories: saved.memories ?? [], animationSlots: saved.animationSlots ?? {}, aiCharacter: saved.aiCharacter ?? false, characterVariants: saved.characterVariants ?? {}, sourcePhoto: saved.sourcePhoto ?? saved.photo });
      }
...
export default function Prototype() {
  ...
  const startGuess = () => setGuessGame({ active: true, round: 0, score: 0, answer: null, reveal: `לאן ${game.name} יקפוץ?` });
  const makeGuess = (choice: "left" | "right") => {
    if (!guessGame.active || guessGame.answer) return;
    const answer = Math.random() > .5 ? "left" : "right";
    const hit = answer === choice;
    const nextRound = guessGame.round + 1;
    const nextScore = guessGame.score + (hit ? 1 : 0);
    setGuessGame({ active: nextRound < 5, round: nextRound, score: nextScore, answer, reveal: hit ? "בול!" : "כמעט!" });
    window.setTimeout(() => {
      if (nextRound >= 5) {
  ...
  const requestCharacterImage = async (references: string[], visual: CharacterVisual) => {
    ...
    if (ai.imageProvider === "kie") {
      ...
      return downloadMedia(await runKieTask(buildKieImageTask(ai.imageModel, buildCharacterPrompt(game.characterKind, game.name, visual), uploaded)));
    }
    if (ai.imageProvider === "fal") {
      return downloadMedia(await runFalTask(ai.imageModel, buildFalImageTask(buildCharacterPrompt(game.characterKind, game.name, visual), references)));
    }
    if (ai.imageProvider === "openai") {
      ...
      form.append("prompt", buildCharacterPrompt(game.characterKind, game.name, visual));
      form.append("size", "1024x1024"); form.append("quality", "medium"); form.append("output_format", "webp"); form.append("background", "transparent");
      response = await fetch(`${mediaBase()}/images/edits`, { method: "POST", headers: mediaHeaders(false), body: form });
    } else {
      response = await fetch(`${mediaBase()}/images`, { method: "POST", headers: mediaHeaders(), body: JSON.stringify(buildOpenRouterCharacterRequest({ model: ai.imageModel, references, kind: game.characterKind, name: game.name, visual })) });
    }
    if (!response.ok) {
      const detail = await response.text();
      throw new Error(`יצירת הדמות נכשלה (${response.status}) ${detail.slice(0, 120)}`);
    }
  ...
  return (
    <div className={`companion-app theme-${game.theme}`} dir="rtl">
      <MobileScroll key={screen} className="app-screen"><main className="game-page">
        ...
        {screen === "journey" ? <JourneyScreen game={game} stage={stage} evolution={evolution} day={day} personality={personality} careGrade={careGrade} onClaim={claimQuest} /> : null}
        {screen === "bag" ? <BagScreen game={game} onUse={useItem} onBuy={buyItem} /> : null}
      </main></MobileScroll>
...
function HomeScreen({ game, characterUrl, health, stage, evolution, day, needs, lowestNeed, callNeed, currentRoom, reaction, reactionId, wanderX, effect, isNight, aiReady, isSpeaking, activeMotion, clipUrl, onSettings, onSpeak, onAction, onUseMedicine }: any) {
  ...
  return <section className="home-screen">
    <img className="room-background" src={currentRoom.image} alt="" draggable={false} /><div className={`room-vignette ${isNight ? "night" : ""}`} />
...
function BagScreen({ game, onUse, onBuy }: any) {
